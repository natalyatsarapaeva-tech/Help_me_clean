...
const verifySystem = `Ты добрый помощник, который проверяет уборку у ребёнка 7 лет. Твоя задача — честно посчитать, что осталось лишнего на поверхности, и сказать об этом так, чтобы ребёнок захотел доделать. Верни ТОЛЬКО JSON:
{"score":"good","praise":"Да ты почти у цели!","left":[{"label":"бумажки","count":3,"category":"trash","todo":"выбросить три бумажки"},{"label":"кружка","count":1,"category":"dishes","todo":"отнести посуду на место"}]}
...
Правила:
- left — ВСЁ лишнее, что осталось на поверхности, сгруппированное по видам, с количеством. Убрано начисто — верни "left": [].
- count — сколько предметов этого вида осталось (числом).
- category — РОВНО ОДИН id: ${ACTION_IDS.join(', ')}.
- todo — готовая фраза, что с этим сделать, в неопределённой форме и с числительным словами: «выбросить три бумажки», «отнести посуду на место», «повесить кофту». Русские падежи согласуй сам.
- Не выдумывай того, чего на фото нет, и не считай мебель, технику и лампу.
- Засчитано или нет — решает приложение по числам, тебе про это думать не надо.
...
- Если кадр плохой (смазан, не тот ракурс, снято не то место) — не проваливай, верни {"retake":true}.
- Если в кадре человек — верни {"person_detected":true}.
- Если дан эталон — считай лишним то, чего на эталоне нет.`;
...
export async function verify({ imageBefore, imageAfter, task, referenceUrl }) {
  ...
  content.push({ type: 'text', text: `Задание было: ${task || 'убрать поверхность'}. Посчитай, что осталось лишнего, и верни JSON.` });
  return jsonCall(
