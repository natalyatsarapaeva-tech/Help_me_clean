...
Приложение переведено полностью — интерфейс, тексты сканера и то, что помощник
говорит вслух. Языков два: английский (по умолчанию) и русский.

- **Где переключать:** родительская панель на главном экране, карточка «Язык»
  (`index.html` → 👑 Родителям). Плюс дубль на экране входа: родитель, который
  не читает по-русски, не должен сперва угадывать, где кнопка «Войти».
- **Что переключается:** вся разметка, тексты в JS, доменные подписи (категории
  действий, виды очагов, типы комнат, образы, ранги, бонусные задания) и язык
  ОТВЕТА модели — промпты в [`js/ai.js`](js/ai.js) английские, а язык
  человеческих строк (подписи предметов, похвала, подсказки) задаётся отдельной
  директивой. Озвучка (`speechSynthesis`) берёт тот же язык.
- **Где хранится:** `localStorage['tidy.lang']` на устройстве плюс
  `families/{fid}/settings/app.lang` в базе. Выбор устройства сильнее: второй
  планшет открывается на языке семьи, но переключить его отдельно можно.
- **Все строки** — в [`js/i18n.js`](js/i18n.js), по одному словарю на язык.
  Новый язык — это новый словарь и один элемент в `LANGS`; ядро знает только
  стабильные id. Полноту словарей проверяет `tests/i18n.test.mjs` — забытый
  ключ роняет тесты, а не показывает английскую строку посреди русского экрана.
